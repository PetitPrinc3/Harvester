...
MAX_TITLE_SCORE = 100
MAX_LANG_SCORE = 25
MAX_MOVIE_QUALITY_SCORE = 4 # Based on 4 preferences
MAX_SHOW_QUALITY_SCORE = 5  # Based on 5 preferences
MAX_COMPLETENESS_SCORE = 30

MAX_MOVIE_SCORE = MAX_TITLE_SCORE + MAX_LANG_SCORE + MAX_MOVIE_QUALITY_SCORE
MAX_SHOW_RELEASE_SCORE = MAX_TITLE_SCORE + MAX_LANG_SCORE + MAX_SHOW_QUALITY_SCORE + MAX_COMPLETENESS_SCORE
...
class ZTParser:
    ...
    def _parse_results_from_page(self, soup):
        """Helper function to parse results from a single BeautifulSoup object."""
        page_results = []
    ...
    def search(self, title, media_type):
        """Searches the website for a given title, scraping all pages of results."""
        base_search_url = f"{self.base_url}/?p={media_type}&search={urllib.parse.quote_plus(title)}"
        all_results = []
        
        print(f"Querying page 1: {base_search_url}")
        try:
            response = requests.get(base_search_url, timeout=10)
            response.raise_for_status()
        except requests.exceptions.RequestException as e:
            print(f"An error occurred while fetching the first page: {e}")
            return []
        ...
        if last_page > 1:
            print(f"Found {last_page} pages of results. Scraping remaining pages...")
            for page_num in range(2, last_page + 1):
                page_url = f"{base_search_url}&page={page_num}"
                print(f"Querying page {page_num}: {page_url}")
                try:
                    ...
                    print(f"  -> Could not fetch page {page_num}. Skipping. Error: {e}")
                    continue
    ...
    def verify_1fichier_link(self, page_url):
        try:
            ...
            print(f"    -> Could not verify page {page_url}: {e}")
            return None
...
def select_best_movie(parser, results, requested_title):
    print(f"\n--- Selecting best movie for '{requested_title}' ---")
    if not results:
        return None
    ...
    print(f"\nFound {len(scored_results)} potential candidates. Verifying for 1fichier link...")
    for candidate in scored_results:
        result_data = candidate['result']
        print(f"  Checking candidate: {result_data['title']} | Score: {candidate['score']}")
        dl_protect_link = parser.verify_1fichier_link(result_data['url'])
        if dl_protect_link:
            print(f"    -> SUCCESS: Found 1fichier link.")
            ...
            print("    -> FAILED: No 1fichier link found.")
    return None

def select_best_show(parser, results, requested_title, requested_season):
    print(f"\n--- Selecting best show for '{requested_title}' S{requested_season} ---")
    if not results:
        return None
    ...
    for r in results:
        ...
        if season_match and int(season_match.group(1)) == requested_season:
            r['found_season'] = int(season_match.group(1)) # Store the found season number
    ...
    print(f"Found {len(top_candidates)} potential candidates. Analyzing...")

    scored_candidates = []
    for candidate in top_candidates:
        print(f"  Checking candidate: {candidate['title']} ({candidate['quality']})")
        ...
        if completeness_score == 0:
            print("    -> FAILED: No 1fichier links found.")
            continue
    ...
    release_percentage = round((best_candidate['score'] / MAX_SHOW_RELEASE_SCORE) * 100, 0)

    return {
        "title": requested_title,
        "season": best_candidate['result']['found_season'], # Use the season number from the result title
...
if __name__ == '__main__':
    ...
    if best_movie:
        print("\n--- Best Movie Result (JSON) ---")
        print(json.dumps(best_movie, indent=4))
    ...
    if best_show:
        print("\n--- Best Show Result (JSON) ---")
        print(json.dumps(best_show, indent=4))